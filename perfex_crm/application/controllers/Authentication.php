...
class Authentication extends ClientsController
{
    ...
    {
        // If already logged in as staff, redirect to admin area
        ...
        if ($this->form_validation->run() !== false) {
            ...
            $staff_user = $this->db->get(db_prefix() . 'staff')->row();

            if ($staff_user) {
                // This is a staff member, redirect to admin login
                set_alert('info', 'Admin Login Required. Please login from the admin area.');
                redirect(admin_url('authentication'));
            }

            // Continue with normal client login
