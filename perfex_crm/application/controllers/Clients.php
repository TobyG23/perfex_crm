...
class Clients extends ClientsController
{
    ...
    public function notifications()
    {
        ...
        $data['notifications'] = $this->get_client_notifications();
        $data['total_unread'] = $this->get_client_unread_notifications_count();
     ...
     */
    private function get_client_notifications($limit = 50)
    {
        $contact_id = get_contact_user_id();
        ...
        $this->db->where('(fromclientid = ' . $contact_id . ' OR (fromclientid IS NULL AND link LIKE "%clients/%"))');
        $this->db->or_where('(description LIKE "%project_discussion%" OR description LIKE "%project_comment%" OR description LIKE "%task_comment%")');
     ...
     */
    private function get_client_unread_notifications_count()
    {
        $contact_id = get_contact_user_id();
    ...
    public function get_notifications()
    {
        ...
        $notifications = $this->get_client_notifications(10);
        echo json_encode($notifications);
    }

    /**
     * Approve overall project design
     */
    public function approve_overall_design()
    {
        try {
            if (!$this->input->is_ajax_request()) {
                show_404();
        ...
        if (!$project_id) {
            echo json_encode(['success' => false, 'message' => _l('project_id_required')]);
            return;
        }

        // Verificar que el cliente tenga acceso al proyecto
        $project = $this->projects_model->get($project_id);

        if (!$project || $project->clientid != get_client_user_id()) {
            echo json_encode(['success' => false, 'message' => _l('access_denied')]);
            return;
        }

        // Verificar que el diseño no haya sido ya aprobado
        if (isset($project->overall_design_approved) && $project->overall_design_approved == 1) {
            echo json_encode(['success' => false, 'message' => _l('design_already_approved')]);
            return;
        }
        ...
        $this->db->update(db_prefix() . 'projects', [
            ...
            'overall_design_approved_by' => $contact_id,
            'overall_design_approved_at' => date('Y-m-d H:i:s'),
            'overall_design_approval_ip' => $ip_address
        ]);

        // Obtener el nombre del contacto
        $contact_full_name = get_contact_full_name($contact_id);

        // Registrar en el log de actividad del proyecto
        $log_data = [
            'description_key' => 'client_approved_overall_design',
            'additional_data' => serialize([
                $contact_full_name,
                $comments ? $comments : _l('no_comments')
            ]),
        ...
        // Crear notificación para el staff del proyecto
        $this->load->model('projects_model');
        $members = $this->projects_model->get_project_members($project_id);

        $contact_name = $contact_full_name;
        $notification_link = 'projects/view/' . $project_id . '?group=project_discussions';

        // Lista de staff a notificar
        $staff_to_notify = [];

        // Agregar miembros del proyecto
        if ($members && is_array($members)) {
            foreach ($members as $member) {
                $staff_to_notify[$member['staff_id']] = $member['staff_id'];
            }
        }

        // Siempre agregar al creador del proyecto
        if ($project->addedfrom && $project->addedfrom > 0) {
            $staff_to_notify[$project->addedfrom] = $project->addedfrom;
        }

        // Si no hay nadie específico, notificar a todos los admin
        if (empty($staff_to_notify)) {
            $this->db->where('admin', 1);
            $this->db->where('active', 1);
            $admins = $this->db->get(db_prefix() . 'staff')->result();
            foreach ($admins as $admin) {
                $staff_to_notify[$admin->staffid] = $admin->staffid;
            }
        }

        // Enviar notificaciones a todos los staff identificados
        $notifications_sent = 0;

        // Cargar modelos y helpers una sola vez fuera del bucle
        $this->load->model('staff_model');
        $this->load->helper('email');

        foreach ($staff_to_notify as $staff_id) {
            // Agregar notificación directamente en la base de datos
            $notification_data = [
                'description' => 'not_client_approved_project_design',
                'touserid' => $staff_id,
                'fromclientid' => $contact_id,
                'link' => $notification_link,
                'additional_data' => serialize([$project->name]),
            ...
            // Enviar push notification a la app móvil (si está configurado)
            if (file_exists(APPPATH . 'libraries/Push_notification.php')) {
                $this->load->library('push_notification');

                $push_title = 'Project Design Approved';
                $push_body = $contact_name . ' approved the design for project: ' . $project->name;

                if ($comments) {
                    $push_body .= ' - Comments: ' . substr($comments, 0, 100);
                }
            ...
            // Enviar notificación por email al staff
            $staff_member = $this->staff_model->get($staff_id);

            if ($staff_member) {
                $merge_fields = [
                    '{staff_firstname}' => $staff_member->firstname,
                    '{client_name}' => $contact_name,
                    '{project_name}' => $project->name,
                    '{approval_date}' => date('Y-m-d H:i:s'),
                    '{approval_comments}' => $comments ? $comments : _l('no_comments'),
                    '{project_link}' => site_url($notification_link)
                ];

                send_mail_template('project_design_approved_to_staff', $staff_member->email, $contact_id, $merge_fields);
            }
        }

        echo json_encode([
            'success' => true,
            'message' => _l('design_approved_successfully')
        ]);

        } catch (Exception $e) {
            log_message('error', 'Exception in approve_overall_design: ' . $e->getMessage());
            ...
            log_message('error', 'PHP Error in approve_overall_design: ' . $e->getMessage());
